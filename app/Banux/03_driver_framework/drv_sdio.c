...
static int sdio_drv_read(void *priv, uint8_t *buf, uint32_t len)
{
    ...
    if (BSP_SD_ReadBlocks((uint32_t *)tmp, p->blockIndex, 1u, SD_DATATIMEOUT) != MSD_OK) {
        return -1;
    }
    if (len > blkSize) {
        len = blkSize;
...
static int sdio_drv_write(void *priv, const uint8_t *buf, uint32_t len)
{
    ...
    memset(tmp, 0xFF, sizeof(tmp)); /* 未写入字节保持擦除态 0xFF */
    memcpy(tmp, buf, len);
    if (BSP_SD_WriteBlocks((uint32_t *)tmp, p->blockIndex, 1u, SD_DATATIMEOUT) != MSD_OK) {
        return -1;
    }
    return (int)len;
}
